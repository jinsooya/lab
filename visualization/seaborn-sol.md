...
**kdeplot** 메소드로 출력하는 도표는 하나이므로 x축과 y축을 라벨링할 때 **pyplot.xlabel** 과 **pyplot. ylabel** 메소드를 호출하면 된다.
...
# stripplot이 반환하는 객체인 AxesSubplot을 'ax'라는 변수로 할당한다.
ax = seaborn.stripplot( data=mpg, x='origin', y='horsepower', 
                       hue='cylinders', jitter=0.4, size=6.5)
...
seaborn.stripplot(data=mpg, x='origin', y='horsepower', color='silver')

# boxplot이 반환하는 객체인 AxesSubplot을 'ax'라는 변수로 할당한다.
ax = seaborn.boxplot(data=mpg, x='origin', y='horsepower')
...
ax = seaborn.swarmplot(data=mpg, x='origin', y='horsepower')
...
# swarmplot이 반환하는 객체인 AxesSubplot을 'ax'라는 변수로 할당한다.
ax = seaborn.swarmplot(data=tips, x='day', y='total_bill')
...
seaborn.swarmplot(data=tips, x='day', y='total_bill', color='silver')

# violinplot이 반환하는 객체인 AxesSubplot을 'ax'라는 변수로 할당한다.
ax = seaborn.violinplot(data=tips, x='day', y='total_bill')
...
seaborn.scatterplot(data=tips, x='total_bill', y='tip', hue='smoker')
...
seaborn.heatmap(flights_pivot, annot=True, fmt='d')

# pyplot.ylim(12, 0)으로 설정하여 히트맵의 y축의 범위가 1949부터 1960이 되도록 한다.
...
pyplot.figure(figsize=(10, 8))
seaborn.heatmap(flights_pivot, annot=True, fmt='d', cmap='Greens')
...
seaborn.heatmap(flights_pivot, annot=True, fmt='d', cmap='BuPu', 
                annot_kws={'size': 14}, linewidths=4)

# 히트맵 출력 전에 pyplot.ylim(12, 0)을 설정하여 히트맵의 y축의 범위가 1949부터 1960이 되도록 한다.
# pyplot.ylim(12,0)이 없으면 위에서 작성된 피벗 테이블의 형태 그대로 히트맵이 작성되어 y축의 시작(즉, 좌측 하단)이 1960이 된다.
...
g = seaborn.jointplot(data=iris, x='sepal_length', y='petal_length')

# 조인트 플롯의 반환한 JointGrid의 set_axis_labels()를 사용하여 x축과 y축을 라벨링한다.
g.set_axis_labels(xlabel='꽃받침 길이', ylabel='꽃잎 길이')
...
g = seaborn.jointplot(data=iris, x='sepal_length', y='petal_length', 
                      color='k').plot_joint(seaborn.kdeplot)
...
**catplot** 메소드에서 매개변수 ***row*** 를 사용하여 캣 플롯이 ***row*** 의 범주에 따라 여러 개의 서브 플롯들로 그릴 수 있다. 그러므로 전체 도표에 대해 제목을 짓기 위해서는 **pyplot.suptitle** 을 사용해야 한다. 여러 개의 서브 플롯이 그려지면 캣 플롯이 반환하는 객체를 변수에 할당하고 **axes** 속성을 이용하여 라벨링을 할 수 있다. 그려지는 서브 플롯의 순서에 따라 **axes[x, y].set_xlabel**, **axes[x, y].set_ylabel** 메소드로 라벨링을 한다. 여기서 **[x, y]** 은 몇 번째 서브 플롯인지를 설정한다. ***x*** 가 행이 되고 ***y*** 가 열이 되는데 예를 들어 **[0, 3]** 이라면 첫 번째 행의 네번째 열을 의미한다.
...
# catplot이 반환하는 객체인 FacetGrid를 'g'라는 변수로 할당한다.
g = seaborn.catplot(data=tips, x='sex', y='total_bill', 
                    hue='smoker', row='day', kind='violin')
...
g.axes[3, 0].set_xlabel('요일')
g.axes[0, 0].set_ylabel('목요일 청구서 금액')
g.axes[1, 0].set_ylabel('금요일 청구서 금액')
g.axes[2, 0].set_ylabel('토요일 청구서 금액')
g.axes[3, 0].set_ylabel('일요일 청구서 금액')

# FacetGrid 객체 'g'의 axes로 각 서브플롯을 호출하고 
# 여기에 set_title()을 사용하여 서브플롯의 제목을 짓는다.
g.axes[0, 0].set_title('목요일')
g.axes[1, 0].set_title('금요일')
g.axes[2, 0].set_title('토요일')
g.axes[3, 0].set_title('일요일')
...
g = seaborn.pairplot(iris)

# g의 axes[x, y].set_xlabel()과 axex[x, y].set_ylabel()로 각 축을 라벨링한다.
g.axes[0, 0].set_ylabel('꽃받침 길이')   # 가장 위의 가장 왼쪽에 있는 subplot의 y축을 라벨링한다.
g.axes[1, 0].set_ylabel('꽃받침 너비')   # 위에서 두 번째, 가장 왼쪽에 있는 subplot의 y축을 라벨링한다.
g.axes[2, 0].set_ylabel('꽃잎 길이')    # 위에서 세 번째, 가장 왼쪽에 있는 subplot의 y축을 라벨링한다.
g.axes[3, 0].set_ylabel('꽃잎 너비')    # 위에서 네 번째, 가장 왼쪽에 있는 subplot의 y축을 라벨링을 한다.

g.axes[-1, 0].set_xlabel('꽃받침 길이')  # 가장 아래의 가장 왼쪽에 있는 subplot의 x축을 라벨링한다.
g.axes[-1, 1].set_xlabel('꽃받침 너비')  # 가장 아래의, 왼쪽에서 두 번째에 있는 subplot의 x축을 라벨링한다.
g.axes[-1, 2].set_xlabel('꽃잎 길이')   # 가장 아래의, 왼쪽에서 세 번째에 있는 subplot의 x축을 라벨링한다.
g.axes[-1, 3].set_xlabel('꽃잎 너비')   # 가장 아래의, 왼쪽에서 네 번째에 있는 subplot의 x축을 라벨링한다.
...
# 페어 플롯이 반환하는 객체를 'g'라는 변수에 할당한다. 
g = seaborn.pairplot(iris, hue='species', markers=['o', 'D', 'x'])
# g = seaborn.pairplot(iris, hue='species', markers=['o', 's', 'D'])  # s는 네모
...
g.axes[3, 0].set_xlabel('꽃받침 길이')  # 가장 아래의 가장 왼쪽에 있는 subplot의 x축을 라벨링한다.
g.axes[3, 1].set_xlabel('꽃받침 너비')  # 가장 아래의, 왼쪽에서 두 번째에 있는 subplot의 x축을 라벨링한다.
g.axes[3, 2].set_xlabel('꽃잎 길이')    # 가장 아래의, 왼쪽에서 세 번째에 있는 subplot의 x축을 라벨링한다.
g.axes[3, 3].set_xlabel('꽃잎 너비')    # 가장 아래의, 왼쪽에서 네 번째에 있는 subplot의 x축을 라벨링한다.
...
# warning 메시지가 출력되어도 정상적으로 스타일이 리셋된다.
import warnings
warnings.simplefilter(action='ignore')
seaborn.reset_orig()
sinplot()
